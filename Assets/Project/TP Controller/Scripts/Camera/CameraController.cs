...
public class CameraController : MonoBehaviour
{
    ...
    public bool useInputForRotation = true;
    ...
    protected virtual void Update()
    {
        ...
        if (useInputForRotation)
        {
            if (!UIElements.Instance.canUseJoystick)
            {
                if (Input.GetMouseButton(1) || Input.GetKey(KeyCode.M))
                {
                    controlRotation = PlayerInput.GetMouseRotationInput(PlayerInput._horizontalAxisCam,PlayerInput._verticalAxisCam);
            ...
            {
                 controlRotation = PlayerInput.GetMouseRotationInputJoystick(PlayerInput._horizontalAxisCam, PlayerInput._verticalAxisCam);
                this.UpdateRotation(controlRotation);
            }
        }
    ...
    public void UpdateRotation(Quaternion controlRotation)
    {
        if (!photonView.IsMine)
            return;
